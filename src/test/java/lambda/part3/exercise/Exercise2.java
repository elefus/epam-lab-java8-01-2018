...
public class Exercise2 {
    ...
    private static class MapHelper<T> {
        ...
        public <R> MapHelper<R> flatMap(Function<T, List<R>> flatMapping) {
            List<R> result = new ArrayList<>();
            source.forEach(value -> {
                List<R> list = flatMapping.apply(value);
                result.addAll(list);
            });
            return new MapHelper<>(result);
        }
    ...
    public void mapEmployeesToCodesOfLetterTheirPositionsUsingMapHelper() {
        ...
        List<Integer> codes = MapHelper.from(employees)
                             ...
                             .flatMap(Exercise2::getCharsAsList)
                             .map(letter -> (int) letter)
                             .getMapped();
        assertEquals(calcCodes("dev", "dev", "tester", "dev", "dev", "QA", "QA", "dev", "tester", "tester", "QA",
                "QA", "QA", "dev"), codes);
    }

    private static List<Character> getCharsAsList(String str) {
