...
public class Exercise1 {
        ...
        List<Person> personsEverWorkedInEpam = employees
                ...
                .filter(
                        employee -> employee
                                .getJobHistory()
                                .stream()
                                .anyMatch(jhe -> { return jhe.getEmployer().equals("EPAM"); })
                )
    ...
    public void findMinimalAgeOfEmployees() {
        ...
        // TODO реализация
        Integer minimalAge = employees.stream().mapToInt(e -> e.getPerson().getAge()).min().orElseThrow(IllegalStateException::new);
