...
public class Exercise2 {
    ...
    public void employersStuffList() {
        ...
        Map<String, Set<Person>> result = employees.stream()
                                                   .flatMap(getPersonEmployerDuration())
                                                   .collect(groupingBy(PersonEmployerDuration::getEmployer,
                                                                       mapping(PersonEmployerDuration::getPerson,
                                                                               toSet())));
    ...
    private Function<Employee, Stream<? extends PersonEmployerDuration>> getPersonEmployerDuration() {
        return e -> e.getJobHistory()
                     .stream()
                     .map(jobHistoryEntry -> new PersonEmployerDuration(e.getPerson(),
    ...
    public void indexByFirstEmployer() {
        ...
        Map<String, Set<Person>> result = employees.stream()
                                                   .collect(groupingBy(e -> e.getJobHistory()
                                                                             .get(0)
                                                                             .getEmployer(),
                                                                       mapping(Employee::getPerson,
                                                                               toSet())));
        ...
        Map<String, Person> collect =
                employees.stream()
                         .flatMap(e -> e.getJobHistory()
                                        ...
                                        .entrySet()
                                        .stream()
                                        .map(ed -> new PersonEmployerDuration(e.getPerson(),
                                                                              ed.getKey(),
                                                                              ed.getValue())))
                         .collect(groupingBy(PersonEmployerDuration::getEmployer,
                                             collectingAndThen(
                                                     maxBy(Comparator.comparingInt(PersonEmployerDuration::getDuration)),
                                                     ped -> ped.orElseThrow(NoSuchElementException::new)
                                                               .getPerson())));
